...
namespace IntroduccionAEFCore1.Controllers
{
    ...
    public class GenerosControllers : ControllerBase
    {
        ...
        public async Task<ActionResult> Post(GeneroCreacionDTO generoCreacion)
        {
            ...
            var yaexisteGenero = await context.Generos.AnyAsync(g => g.Nombre == generoCreacion.Nombre);
            if (yaexisteGenero)
            {
        ...
        //modelo conectado para actualizar dato
        ...
        public async Task<ActionResult> DeleteAnteriot(int id)
        {
            var genero = await context.Generos.FirstOrDefaultAsync(g => g.Id == id);
            if (genero is null)
            {
